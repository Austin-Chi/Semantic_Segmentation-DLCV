...
#Configuration
logging.basicConfig(filename='DEEPLAB_RESNET50.txt', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
checkpoint_dir = './p2/checkpoints_DEEPLAB_RESNET50'
os.makedirs(checkpoint_dir, exist_ok=True)
log_file = 'p2/deeplab_RESNET50_log.csv'
train_dir = './train'
val_dir = './validation'
num_epochs = 450
num_classes = 7
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        pt = torch.exp(-ce_loss)  # pt = e^(-ce_loss)
...
checkpoint = torch.load('p2/checkpoints_DEEPLAB_RESNET50/deeplab_res50_best_epoch_248.pth')
...
for epoch in range(751, 751+num_epochs):
    ...
    if val_loss > best_val_loss:
        ...
        checkpoint_path = os.path.join(checkpoint_dir, f'deeplab_res50_best_epoch_{epoch+1}.pth')
        save_checkpoint(epoch + 1, model, optimizer, val_loss, checkpoint_path)
        torch.save(model, 'p2/best_deeplab_res50.pth')
